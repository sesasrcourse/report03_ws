...
\begin{itemize}
    ...
    \item \texttt{/goal\_marker} (Marker): Visualizes the current goal in RViz
...
\begin{enumerate}
    ...
    \item \textbf{Angular sectoring}: The full 360° scan (could be less in real robot) is divided into $N_{ranges} = 20$ angular sectors to reduce computational load
...
This filtering approach reduces the number of obstacle points to 20, significantly improving computational performance while maintaining sufficient environmental representation for safe navigation.
...
A timeout mechanism resets the goal if no landmark is detected, preventing the robot from pursuing stale targets.
...
\begin{enumerate}
    \item \textbf{Collision detection}: If any LiDAR measurement falls below the collision tolerance (0.25 m), the robot immediately stops and sets a collision flag. If the obstacle is no longer detected, navigation resumes.
...
The DWA controller generates trajectories by sampling the dynamic window of feasible velocities and evaluating them according to an objective function. Three variants were implemented:

\subsubsection{Task 1: Original DWA}
The standard objective function balances three terms:
\begin{equation}
...
Key parameters were tuned as: $\alpha = 0.1$, $\beta = 0.2$, $\gamma = 0.2$, with velocity samples $N_v = 20$ and angular velocity samples $N_\omega = 20$, resulting in 400 evaluated trajectories per control cycle.

\subsubsection{Task 2a: Velocity Reduction Near Goal}
An additional penalty term was introduced to slow down the robot when approaching the goal:
\begin{equation}
    vel' = -v \cdot \left(1 - \frac{d_{goal}}{d_{threshold}}\right), \quad \text{if } d_{goal} < d_{threshold}
\end{equation}

where $d_{threshold} = 0.25$ m defines the activation range. The penalty increases linearly as the robot gets closer to the goal, effectively reducing the selected velocities in the final approach phase.

\subsubsection{Task 2b: Target Following}
For dynamic target following, a distance maintenance term was added:
\begin{equation}
...
with $d_{desired} = 0.3$ m. Additional penalties (50\% reduction if $d < 0.5 \cdot d_{desired}$, 70\% if $d > 2.0 \cdot d_{desired}$) discourage getting too close or too far from the target, ensuring optimal visual tracking conditions.
...
\begin{enumerate}
    \item Check goal reach condition: $\|goal_{robot}\| < d_{tol}$ (0.2 m)
