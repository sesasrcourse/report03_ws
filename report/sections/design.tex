...
\begin{itemize}
    ...
    \item \texttt{/goal\_marker} (Marker): Used to visualize the current goal in RViz (see \autoref{fig:filter_scan})
    ...
    \item \texttt{/filter\_scan} (MarkerArray): Visualizes filtered LiDAR points (see \autoref{fig:filter_scan})
...
\singlepic{filter_scan.png}{Rviz visualization of \texttt{/filter\_scan} (blue spheres) and \texttt{/goal\_marker} (green cube) topics}{filter_scan}{0.5}
...
\begin{equation}
    \begin{bmatrix} x_{goal,robot} \\ y_{goal,robot} \end{bmatrix} = 
    \begin{bmatrix} \cos(-\theta) & -\sin(-\theta) \\ \sin(-\theta) & \cos(-\theta) \end{bmatrix}
    \begin{bmatrix} x_{goal,global} - x_{robot,global} \\ y_{goal,global} - y_{robot,global} \end{bmatrix}
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Timeout protection}: When the robot cannot see the goal anymore or cannot reach it - unless the goal dynamically changes - for more than 300 control steps (circa 20 $s$), a timeout flag is triggered.
...
where $d_{threshold}$ defines the activation range. The penalty increases linearly as the robot gets closer to the goal, effectively reducing the selected velocities in the final approach phase.
...
\begin{enumerate}
    \item Check goal reach condition: $\|[x_{goal,robot} \quad y_{goal,robot}]\| < d_{tol}$
