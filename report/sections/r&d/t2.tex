...
The addition of the distance regulation term (Task 2b) causes a different behavior compared to the default objective function 1, as can be seen in \autoref{tab:metrics} by an increase in distance RMSE. This is due to the DWA prioritizing the maintenance of the desired distance from the goal when the robot is close, penalizing high velocities that could create deviations from that target distance and consequently making the overall motion slower.
The bearing RMSE is not remarkably affected, suggesting that the added constraint introduces minor trade-offs in tracking precision. However, success and tracking rates remain at 100\%, confirming robust navigation performance.
